...
if ~contains(path, generatedDataDir)
    addpath(generatedDataDir)
end
if ~contains(path, simDataDir)
    addpath(simDataDir)
end

% ensure required sim input exists
if ~exist(fullfile(simDataDir,'sous marin en pentagone.mat'),'file')
    error('Fichier attendu non trouvé : %s', fullfile(simDataDir,'sous marin en pentagone.mat'))
end
...
% runWorkflow is a function, so this script runs in the function workspace.
% Simulink Constant blocks in the model evaluate these values from the base
% workspace, so export the model parameters explicitly before sim().
modelParameterNames = {
    'x_rg'
    'y_rg'
    'z_rg'
...
for parameterIndex = 1:numel(modelParameterNames)
    parameterName = modelParameterNames{parameterIndex};
    assignin('base', parameterName, eval(parameterName));
end
